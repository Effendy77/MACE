...
def evaluate(data_loader, model, device, task, epoch, mode, num_class):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            prediction_list.extend(prediction_softmax[:, 1].cpu().detach().numpy())  # Take only the probability of positive class
    ...
    # Calculate metrics
    accuracy_score_ = accuracy_score(true_label_decode_list, prediction_decode_list)
    auc_roc = roc_auc_score(true_label_decode_list, prediction_list)
    f1_score_ = f1_score(true_label_decode_list, prediction_decode_list)
    mcc = matthews_corrcoef(true_label_decode_list, prediction_decode_list)

    metric_logger.synchronize_between_processes()

    print('Sklearn Metrics - Acc: {:.4f} AUC-roc: {:.4f} F1-score: {:.4f} MCC: {:.4f}'.format(
        accuracy_score_, auc_roc, f1_score_, mcc))

    # Save metrics to a CSV file
    results_path = task + '_metrics_{}.csv'.format(mode)
    with open(results_path, mode='a', newline='', encoding='utf8') as cfa:
        wf = csv.writer(cfa)
        data2 = [[accuracy_score_, auc_roc, f1_score_, mcc, metric_logger.loss]]
        for i in data2:
            wf.writerow(i)

    # Plot confusion matrix and ROC curve for test mode
    if mode == 'test':
        ...
        fpr, tpr, thresholds = roc_curve(true_label_decode_list, prediction_list)
        roc_auc = auc(fpr, tpr)
        ...
        plt.savefig(task + 'ROC_Curve.jpg', format='jpg')
        plt.close()
